...
void draw() {
  ...
  if (!shownIntro) {
    textFields.add(new TextField(
      "Piloto! Recuerda, te mueves con el PadDireccional!"
      ,5000));
    shownIntro = true;
  }
  ...
  if (scores.score >= 20 && !shownBossPoint) {
    textFields.add(new TextField(
      "HEY! No te distraigas, ese solo ha sido el primero!"
      ,5000));
    shownBossPoint = true;
  }
...
void spawnController() {
  ...
  if (boss != null && !boss.active && scores.score >= 150) {
    textFields.add(new TextField(
      "Detecto una presencia superior! Ten cuidado!"
      ,5000));
    boss.active = true;
  }
